...
class SyncProjectsUseCaseTest {

    private val dispatcher = UnconfinedTestDispatcher()

    private lateinit var useCase: SyncProjectsUseCase
    ...
    @Before
    fun setUp() {
        useCase = SyncProjectsUseCase(projectRepository, getPortfolioApiServiceErrorUseCase)
    }

    @Test
    fun `GIVEN the happy path WHEN invoke THEN projectRepository_getProjects(true) should be called once`() {
        val projects = listOf(buildProject())

        coEvery { projectRepository.getProjects(true) } returns projects.right()
